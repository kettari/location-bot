...
func (he *HtmlEngine) Process(page *scraper.Page) (*[]entity.Game, error) {
	...
	processAllNodes = func(n *html.Node) {
		...
		if n.Type == html.ElementNode && n.Data == "div" && he.isDivEvent(n) {
			...
			// Set joinable flag
			if game.Date.After(time.Now()) && game.SeatsTotal > 0 && game.SeatsFree > 0 {
				game.Joinable = true
			} else {
				game.Joinable = false
			}
	...
	slog.Debug("found games", "games_count", len(games))
...
func (he *HtmlEngine) processEventNode(n *html.Node, game *entity.Game) {
	...
	case "h4":
		if he.attrValue(n.Attr, "class") == "game-title" && n.FirstChild != nil {
			game.Title = he.findWeekendEventTitle(n.FirstChild)
		} else {
...
func (he *HtmlEngine) findWeekendEventTitle(n *html.Node) string {
	...
	if n.NextSibling != nil {
		return he.findWeekendEventTitle(n.NextSibling)
	}
	return ""
}
...
func (he *HtmlEngine) parseWeekendDateNode(n *html.Node, slots *dateSlots) {
	if n.Type == html.ElementNode && he.attrValue(n.Attr, "class") == "caption" {
		// Суббота — 19.04.2025
		r := regexp.MustCompile(`^[\x{0400}-\x{04FF}]+\s.\s(\d{2})\.(\d{2})\.(\d{4})`)
		matches := r.FindAllStringSubmatch(n.Data, -1)
		if len(matches) > 0 {
			moscow, err := time.LoadLocation("Europe/Moscow")
			if err != nil {
				panic(err)
...
func (he *HtmlEngine) parseTimeSlots(n *html.Node) {}
